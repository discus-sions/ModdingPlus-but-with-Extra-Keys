...
class ChartingState extends MusicBeatState
{
	...
	**/
	...
	//starts from ten to avoid other note types
	...
	override function create() {
		...
		modeTxt = new FlxText(100, 50, 0, "Mode: Delete\nUse Ctrl to change", 16);
	...
	function addSectionUI():Void {
		...
		var copyButton:FlxButton = new FlxButton(160, 70, "Copy last section", function() {
			if (Std.int(stepperSection.value) != 0)
		...
		var swapSection:FlxButton = new FlxButton(10, 120, "Swap section", function() {
			for (i in 0..._song.notes[curSection].sectionNotes.length) {
				...
				var baseNote = (note[1] + 4) % 8;
				var specialValue = note[1] - ((baseNote + 4) % 8); //lol
				if (specialValue < 0)
					specialValue = 0;
	...
	var stepperNoteChar:FlxUINumericStepper;

	var noteTypes:Array<String> = ['Normal', 'Fire', 'Death', 'Warning', 'Angel', 'Alt Anim', 'Bob', 'Glitch'];
	var typeChangeLabel:FlxText;

	var charList:Array<String> = ['Shaggy', 'Matt', 'Ron', 'Bambi', 'Bob', 'Tankman', 'Dave', 'Tricky', 'Little-Man', 'Cheeky'];
									//bambi, ron, shaggy, matt, tankman, dave, bob, tricky
	var typeChangeCharLabel:FlxText;
	function addNoteUI():Void {
		...
		var noteTypeButton:FlxButton = new FlxButton(10, 130, "Change Type", function() {
			curSelectedNote[1] %= 8; // this looks so strange but it works
			switch (noteType) {
				case Mine: 
					curSelectedNote[1] += 8;
				case Lift: 
					curSelectedNote[1] += 16;
				case Death: 
					curSelectedNote[1] += 24;
				case key: 
					curSelectedNote[1] += 8 * key;
			}
			updateGrid();
		});
		...
		stepperNoteChar = new FlxUINumericStepper(100, 155, 1, selectedOne, 0, charList.length - 1, 0);
		stepperNoteChar.value = selectedOne;
		stepperNoteChar.name = 'char_list';

		typeChangeLabel = new FlxText(100, 75, 64, noteTypes[selectedType] + " notes");
		typeChangeCharLabel = new FlxText(100, 175, 64, charList[selectedOne] + " character\n Press Z to Switch");
		...
		var charlabel = new FlxText(100,135,64,'Character Note List');

		var m_check = new FlxUICheckBox(10, 165, null, null, "6", 100);
		m_check.checked = (_song.noteMania == 1);
		...
		m_check2.checked = (_song.noteMania == 2);
		...
		m_check3.checked = (_song.noteMania == 3);
		...
		m_check4.checked = (_song.noteMania == 4);
		...
		m_check5.checked = (_song.noteMania == 5);
		...
		m_check6.checked = (_song.noteMania == 5); 
		...
		m_check7.checked = (_song.noteMania == 7);
		...
		m_check8.checked = (_song.noteMania == 5);
		...
		m_check0.checked = (_song.noteMania == 0);
		...
		tab_group_note.add(stepperNoteChar);
		tab_group_note.add(typeChangeCharLabel);
		tab_group_note.add(charlabel);
	...
	function sectionStartTimeSET(section:Int):Float
	{
	...
	override function update(elapsed:Float) {
		...
		typeChangeCharLabel.text = charList[Std.int(stepperNoteChar.value)] + ' character\n Press Z to Switch';
		curStep = recalculateSteps();
		if (_song.noteMania == 2 && gridBG.width != S_GRID_SIZE * 18)
			{
				remove(gridBG);
				gridBG = FlxGridOverlay.create(S_GRID_SIZE, GRID_SIZE, S_GRID_SIZE * 18, GRID_SIZE * 16);
				add(gridBG);
				remove(gridBGAbove);
				gridBGAbove = FlxGridOverlay.create(S_GRID_SIZE, GRID_SIZE, S_GRID_SIZE * 18, GRID_SIZE * 16);
				add(gridBGAbove);
				remove(gridBGBelow);
				gridBGBelow = FlxGridOverlay.create(S_GRID_SIZE, GRID_SIZE, S_GRID_SIZE * 18, GRID_SIZE * 16);
				add(gridBGBelow);
			}
			if (_song.noteMania == 5 && gridBG.width != S_GRID_SIZE * 16)
				{
					remove(gridBG);
					gridBG = FlxGridOverlay.create(S_GRID_SIZE, GRID_SIZE, S_GRID_SIZE * 16, GRID_SIZE * 16);
					add(gridBG);
					remove(gridBGAbove);
					gridBGAbove = FlxGridOverlay.create(S_GRID_SIZE, GRID_SIZE, S_GRID_SIZE * 16, GRID_SIZE * 16);
					add(gridBGAbove);
					remove(gridBGBelow);
					gridBGBelow = FlxGridOverlay.create(S_GRID_SIZE, GRID_SIZE, S_GRID_SIZE * 16, GRID_SIZE * 16);
					add(gridBGBelow);
				}
			if (_song.noteMania == 4 && gridBG.width != GRID_SIZE * 14)
				{
					remove(gridBG);
			...
			if (_song.noteMania == 1 && gridBG.width != GRID_SIZE * 12)
			{
				remove(gridBG);
			...
			if (_song.noteMania == 3 && gridBG.width != GRID_SIZE * 10)
				{
					remove(gridBG);
			...
			if (_song.noteMania == 0 && gridBG.width != GRID_SIZE * 8)
			{
				remove(gridBG);
				gridBG = FlxGridOverlay.create(GRID_SIZE, GRID_SIZE, GRID_SIZE * 8, GRID_SIZE * 16);
				add(gridBG);
			...
			if (_song.noteMania == 6 && gridBG.width != GRID_SIZE * 2)
				{
					remove(gridBG);
			...
			if (_song.noteMania == 7 && gridBG.width != GRID_SIZE * 4)
				{
					remove(gridBG);
			...
			if (_song.noteMania == 8 && gridBG.width != GRID_SIZE * 6)
				{
					remove(gridBG);
		...
		leftIcon.setPosition(0, -100);
		rightIcon.setPosition(gridBG.width / 2, -100);
		UI_box.x = FlxG.width / 2;// + 160 * _song.noteMania;
		UI_box.y = 20;
		if (_song.noteMania != 0)
		{
			UI_box.x = FlxG.width / 2 + 160;// + 160 * _song.noteMania;
			UI_box.y = 100;
		}
		...
		if (_song.noteMania == 2 || _song.noteMania == 5) strumLine.x = -70;
		...
		if (!typingShit.hasFocus && !player1TextField.hasFocus && !player2TextField.hasFocus && !gfTextField.hasFocus && !stageTextField.hasFocus && !cutsceneTextField.hasFocus && !uiTextField.hasFocus) {
			...
			if (FlxG.keys.justPressed.CONTROL) {
				...
				if (deleteMode)
					modeTxt.text = 'Mode: Delete\nUse Ctrl to change';
				else
					modeTxt.text = 'Mode: Select\nUse Ctrl to change';
			}
		}
	...
	function updateGrid():Void {
		...
		for (i in sectionInfo) {
			...
			note.y = Math.floor(getYfromStrum((daStrumTime - sectionStartTimeSET(curSection)) % (Conductor.stepCrochet * _song.notes[curSection].lengthInSteps)));
		...
		if (curSection != 0)
			{
				for (i in lastSectionInfo)//copy pasted cuz it was broken as shit and crashing and shit and i hate this chart editor please nija mff make good one please
					{
						var daNoteInfo = i[1];
						var daStrumTime = i[0];
						var daSus = i[2];
						...
						note.y = Math.floor(getAboveYfromStrum((daStrumTime - sectionStartTimeSET(curSection - 1)) % (Conductor.stepCrochet * _song.notes[curSection - 1].lengthInSteps)));
						//note.y += gridBG.height;
			
						if (_song.noteMania == 2 || _song.noteMania == 5)
						{
			...
			for (i in nextSectionInfo)
				{
					...
					note.x = Math.floor(daNoteInfo * GRID_SIZE);
					note.y = Math.floor(getBelowYfromStrum((daStrumTime - sectionStartTimeSET(curSection + 1)) % (Conductor.stepCrochet * _song.notes[curSection + 1].lengthInSteps)));
					//note.y += gridBG.height;
	...
	private function addNote():Void {
		var noteStrum = getStrumTime(dummyArrow.y) + sectionStartTimeSET(curSection);
