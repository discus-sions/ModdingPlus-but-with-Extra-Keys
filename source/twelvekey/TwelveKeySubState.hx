...
class TwelveKeySubState extends MusicBeatSubstate
{
    var key_Count:Int = 4;
    ...
    var fullscreenBind:String = FlxG.save.data.keys.FULLSCREEN;
    var fullscreenText:FlxText = new FlxText();

    var pauseKey:FlxSprite = new FlxSprite();
    var pauseBind:String = FlxG.save.data.keys.PAUSE;
    var pauseText:FlxText = new FlxText();

    var mania_gap:Array<String>;

    public var notSpam:Bool = false;

    public function new()
    {
        FlxG.mouse.visible = true;

        ui_Settings = LeatherUtils.coolTextFile(Paths.txt("leather_engine/ui/config"));
        mania_size = LeatherUtils.coolTextFile(Paths.txt("leather_engine/ui/maniasize"));
        mania_offset = LeatherUtils.coolTextFile(Paths.txt("leather_engine/ui/maniaoffset"));
        mania_gap = LeatherUtils.coolTextFile(Paths.txt("leather_engine/ui/maniagap"));

        arrow_Configs.set("default", LeatherUtils.coolTextFile(Paths.txt("leather_engine/ui/default")));
    ...
    override function update(elapsed:Float) {
        ...
        var reset = FlxG.save.data.keys.RESET;
        ...
        if(arrow_Group != null)
        {
            ...
            if(back)
            {
                ...
                FlxG.save.data.keys.FULLSCREEN = fullscreenBind;
                FlxG.save.data.keys.RESET = killBind;
                FlxG.save.data.PAUSE = pauseBind;
    
                //PlayerSettings.player1.controls.loadKeyBinds();
                //we don't need to do it now, as keybinds are always rebinded when PlayState starts.
    ...
    {
        try {for(i in 0...text_Group.length)
        {
    ...
    function create_Arrows(?new_Key_Count)
    {
        ...
        {
            var babyArrow:TwelveKeyStrum = new TwelveKeyStrum(0, strumLine.y, i, "default", ui_Settings, mania_size, key_Count);

            babyArrow.frames = Paths.getSparrowAtlas("leather_engine/ui/default");
            ...
            var coolWidth = Math.ceil(babyArrow.width / 2.83684); try {

            var coolText = new FlxText((babyArrow.x + (babyArrow.width / 2)) - (coolWidth / 2), 
            babyArrow.y - (coolWidth / 2), coolWidth, 
            binds[key_Count - 1][i], 
            coolWidth);
            add(coolText); 

            text_Group.add(coolText); } catch (e) { trace(e.message + " | " + binds + " | " + coolWidth + " | " + FlxG.save.data.binds); }
        }
    }
